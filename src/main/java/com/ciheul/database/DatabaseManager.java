...
public class DatabaseManager {
	...
	public static void updateBit48(String billNumber1, String billNumber2, String transactionId, String bit48,
		...
		try {
			...
			if (statuss != 1) { // fail
				logger.warn("Failed to update bit48 on transaction " + transactionId);
				conn.rollback();
			}
			conn.commit();
			...
			if (prepStatement != null) {
				try {
					...
					logger.error("Failed to close preparation Statement : " + e.getMessage());
				}
			}
	...
	public static boolean updateStatusTransaction(String transactionId, int status, String resultCode, String message) {
		...
		String updateStatusQ = "UPDATE adm_transaction "
				+ "SET status=?, result_code=?, note=? WHERE transaction_id=? " + "AND status=?";
		...
		try {
			...
			logger.error("Error on updating status of transaction : " + e.getMessage());
			e.printStackTrace();
	...
	public static boolean insertTransaction(String transactionId, String transactionRef, int accountId, int productId,
		...
		String transactionQ = "INSERT INTO adm_transaction " + "(transaction_id, transaction_ref_id, account_id, "
				+ "product_id, bill_number, status, amount, result_code, " + "note, bit_61, bit_48, timestamp) "
				+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss:FF");
	 ...
	 * Delete a STAN
	 */
	public static void deleteStan(String stan) {
		Jedis jedis = null;
		RedisConnection rc = null;
	 ...
	 * Get all STAN
	 * 
	 * @return a list of STAN
	 */
	public static List<String> getAllStan() {
		Jedis jedis = null;
		RedisConnection rc = null;

		List<String> stans = new ArrayList<String>();
		try {
			rc = RedisConnection.getInstance();
			jedis = rc.getConnection();

			Map<String, String> allStan = jedis.hgetAll(Context.STAN);
			for (String s : allStan.keySet()) {
				stans.add(s);
			}
		} catch (JedisConnectionException jce) {
			logger.error("Error on redis connection : " + jce.getMessage());
	...
	/**
	 * Delete STAN key (delete all STAN).
	 */
	public static void deleteAllStan() {
		Jedis jedis = null;
		RedisConnection rc = null;
	...
	/**
	 * Check whether stan is empty or not.
	 * 
	 * @return True or False
	 */
	public static boolean isEmptyStan() {
		Jedis jedis = null;
		RedisConnection rc = null;

		Map<String, String> allStan = null;
		try {
			rc = RedisConnection.getInstance();
			jedis = rc.getConnection();
